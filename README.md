...
- 🏠 **房间名仅用于展示**：「设置房间」里填的房间号/校区/楼栋只存在当前浏览器（localStorage），
  仅用于页面显示，**不参与数据查询**（数据始终来自仓库固定的那间宿舍）；
  换浏览器、或在 `127.0.0.1` 与 `localhost` 之间换地址，都需各设置一次
## 架构
...
- 两种查看共用同一份纯静态前端（`docs/`），数据源为 `docs/data/stats.json`。
...
2. Token name 随意，设有效期建议**No expiration**，仓库访问权限选择 **Only select repositories** 再选择自己fork的对应仓库；
3. 权限使用 
...
4. 点 Generate，**立即复制**（只显示这一次）；建议存到仓库根目录 `.token` 文件（已 .gitignore，不会公开，`scripts\trigger_action.bat` 会自动读取，方便本地触发）；
...
Settings → Pages → Deploy from a branch → `master` / `/docs` → Save；约 1 分钟后访问 `https://<用户名>.github.io/<仓库名>/`。
...
往 `data/electricity_data.csv` 追加一行 `时间,数值,度`（如 `2026-08-28T21:00:00,1512.1,度`），
本地重跑 `uv run --no-project python src\stats.py` 即可预览；push 后线上同步。
...
# ② 手动触发云端采集（用仓库 Secrets 跑，本地零凭据零依赖）
scripts\trigger_action.bat      # token 读自 NJU_POWER_TOKEN 环境变量或根目录 .token 文件
```
